    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  ...
  Q = process_noise_cov();
  #endif
  cov_acc       = V3D(0.1, 0.1, 0.1); // 加速度测量协方差
  cov_gyr       = V3D(0.1, 0.1, 0.1); // 角速度测量协方差
  cov_bias_gyr  = V3D(0.0001, 0.0001, 0.0001); // 角速度测量协方差偏置
  cov_bias_acc  = V3D(0.0001, 0.0001, 0.0001); // 加速度测量协方差偏置
  mean_acc      = V3D(0, 0, -1.0);
  mean_gyr      = V3D(0, 0, 0);
  angvel_last     = Zero3d; // 上一帧角速度
  Lidar_T_wrt_IMU = Zero3d; // 平移外参
  Lidar_R_wrt_IMU = Eye3d; // 旋转外参
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  for (const auto &imu : meas.imu) // 计算方差
  {
    ...
    cov_acc = cov_acc * (N - 1.0) / N + (cur_acc - mean_acc).cwiseProduct(cur_acc - mean_acc) * (N - 1.0) / (N * N); // .cwiseProduct()对应系数相乘
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  double back_point_time = disable_undistort?0.1:pcl_out.points.back().curvature / double(1000);  //拿到最后一帧时间戳加上最后一帧的所需要的时间/1000得到点云的结束时间戳
  ...
  /*** Initialize IMU pose ***/
  state_ikfom imu_state = kf_state.get_x(); // 获取上一次KF估计的后验状态作为本次预测的初始状态
  IMUpose.clear();
